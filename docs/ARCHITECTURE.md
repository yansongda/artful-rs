...
pub fn get_client() -> &'static reqwest::Client {
    ...
    CLIENT.get_or_init(|| {
        let http = Artful::get_config().http.clone();
        build_client(http)
    })
}

fn build_client(http: HttpOptions) -> reqwest::Client {
    let pool_idle_timeout = http.pool_idle_timeout.unwrap_or(90);
    let pool_max_idle_per_host = http.pool_max_idle_per_host.unwrap_or(20);
